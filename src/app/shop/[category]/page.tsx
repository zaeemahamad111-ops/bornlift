  ...
  return (
    <main className="pt-24 md:pt-32 min-h-screen bg-black">
      <section className="px-6 md:px-8 max-w-[1600px] mx-auto mb-10 md:mb-16">
        <span className="font-label-technical text-label-technical text-[#C0FF00] uppercase mb-4 block">// CATEGORY_PROTOCOL</span>
        ...
        <div className="flex flex-wrap gap-3 md:gap-4 border-b border-zinc-900 pb-8">
          <Link href="/shop" className="px-4 md:px-6 py-2 border border-zinc-800 text-zinc-400 font-label-technical text-xs md:text-sm hover:border-white hover:text-white transition-all">ALL_GEAR</Link>
      ...
      <section className="px-8 max-w-[1600px] mx-auto pb-40">
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-20 border border-zinc-900 bg-zinc-950/30">
            ...
            <p className="font-label-technical text-zinc-500 uppercase tracking-widest">No specimens found in this sector.</p>
            <Link href="/shop" className="inline-block mt-8 text-[#C0FF00] font-label-technical underline underline-offset-8">RETURN_TO_BASE</Link>
          </div>
        )}
      </section>
    </main>
